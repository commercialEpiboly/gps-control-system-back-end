...
public class JT808Endpoint {
    ...
    public Object generalResponse(T0001 message, Session session) {
        log.info("终端通用应答参数:{},----:{}", JSONUtil.toJsonStr(message), JSONUtil.toJsonStr(session));
    ...
    public void heartbeat(JTMessage message, Session session) {
        log.info("终端心跳参数:{},----:{}", JSONUtil.toJsonStr(message), JSONUtil.toJsonStr(session));

    }

    @Mapping(types = 终端注销, desc = "终端注销")
    public void unregister(JTMessage message, Session session) {
        log.info("终端注销参数:{},----:{}", JSONUtil.toJsonStr(message), JSONUtil.toJsonStr(session));
    ...
    public T8100 register(T0100 message, Session session) {
        log.info("终端注册参数:{},----:{}", JSONUtil.toJsonStr(message), JSONUtil.toJsonStr(session));
    ...
    public T0001 authentication(T0102 message, Session session) {
        log.info("终端鉴权参数:{},----:{}", JSONUtil.toJsonStr(message), JSONUtil.toJsonStr(session));
    ...
    public void parametersResponse(T0104 message, Session session) {
        log.info("查询终端参数应答参数:{},----:{}", JSONUtil.toJsonStr(message), JSONUtil.toJsonStr(session));
    ...
    public void locationReport(List<T0200> list) {
        log.info("车辆位置信息汇报参数参数:{}", JSONUtil.toJsonStr(list));

        saveGpsInfo(list, null);
    }

    @Mapping(types = 定位数据批量上传, desc = "定位数据批量上传")
    public void locationBatchReport(T0704 message) {
        log.info("车辆定位数据批量上传参数:{}", JSONUtil.toJsonStr(message));
        saveGpsInfo(message.getItems(), message.getClientId());

    }

    protected void saveGpsInfo(List<T0200> list, String deviceId) {
        List<SysCarGpsInfo> gpsInfoList = list.stream().map(t0200 -> {
            SysCarGpsInfo carGpsInfo = SysCarGpsInfo.builder().build();
            carGpsInfo.setDeviceId(StringUtils.isBlank(t0200.getClientId()) ? deviceId : t0200.getClientId());
            carGpsInfo.setLatitude(String.valueOf(t0200.getLatitude()));
            carGpsInfo.setLongitude(String.valueOf(t0200.getLongitude()));
            carGpsInfo.setSpeed(t0200.getSpeed());
            LocalDateTime ldt = LocalDateTime.now();
            try {
                ldt = LocalDateTime.parse("20".concat(t0200.getDateTime()), DTF);
            } catch (Exception e) {
                log.error("位置上报时间转换错误！", e);
            }
            carGpsInfo.setCreateDateTime(ldt);
            return carGpsInfo;
        }).collect(Collectors.toList());
        List<SysCarGpsInfo> carGpsInfoList = gpsInfoList.stream().distinct().collect(Collectors.toList());
        log.info("执行保存置信息:{}", JSONUtil.toJsonStr(carGpsInfoList));
        carDeviceGpsService.saveBatch(carGpsInfoList);
    }
